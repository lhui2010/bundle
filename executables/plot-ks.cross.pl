...
$input_is_dir++ if (-d $qry);
if (!$input_is_dir) {
    $ref = shift;
    $transcriptome = $qry.'-'.$ref;
    ...
    die "Could not locate '$qry.pep $qry.cds $ref.cds $ref.pep '.\n".&usage if (!-e "$qry.cds" or !-e "$qry.pep" or !-e "$ref.cds" or !-e "$ref.pep");
}
else{
    $transcriptome = $qry;
    ($qry, $ref) = split/-/, $transcriptome;
}
...
if (!$input_is_dir) {
	...
	my $qry_abs_path = abs_path($qry);
	my $ref_abs_path = abs_path($ref);
    ...
    #$transcriptome =~ s/.*\///;
	run_cmd("ln -s $qry_abs_path.pep $ENV{PWD}/$project_name/$qry.pep");
	run_cmd("ln -s $ref_abs_path.pep $ENV{PWD}/$project_name/$ref.pep");
	run_cmd("ln -s $qry_abs_path.cds $ENV{PWD}/$project_name/$qry.cds");
	run_cmd("ln -s $ref_abs_path.cds $ENV{PWD}/$project_name/$ref.cds");
	run_cmd("cat $qry_abs_path.pep $ref_abs_path.pep > $ENV{PWD}/$project_name/$transcriptome.transdecoder.pep");
	run_cmd("cat $qry_abs_path.cds $ref_abs_path.cds > $ENV{PWD}/$project_name/$transcriptome.transdecoder.cds");
}
else {
...
if ($pid == 0) {
	...
	create_ks_plot();
    `ln -s paralogs-t300-mYN.kaks $qry.$ref.ortho.kaks.raw`;
	exit(0);
}
else {
	waitpid($pid, 0);
...
sub parse_self_blat_output_child {
	...
	while (my $line = <$blat_out>) {
		...
		foreach my $align (@query_align) {
            # print(reverse_translate({"DNA" => $query_nuc_align, "PROT" => $align}));
            if($align ne "")
            {
		...
		foreach my $align (@match_align) {
            # print(reverse_translate({"DNA" => $match_nuc_align, "PROT" => $align}));
            if($align ne "")
            {
...
Usage:
perl plot-ks.cross.pl Abrus_precatorius Angylocalyx_braunii_Pap -o Abrus_precatorius.Angylocalyx_braunii_Pap

Require Abrus_precatorius.cds Abrus_precatorius.pep Angylocalyx_braunii_Pap.cds Angylocalyx_braunii_Pap.pep
--- 

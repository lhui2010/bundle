...
def add_depth_to_mosaic(mosaic_bedpe=None, bkptsum_l=None,
                        bkptsum_r=None, with_read_name='F'):
    """
    All is 1-based
    :param mosaic_bedpe:
    ...
    rdnamedb = defaultdict(str)
    rdnamedb_right = defaultdict(str)
    with open(bkptsum_l) as fh:
        # 1       37      Head    1
        for line in fh:
            mylist = line.rstrip().split()
            if with_read_name == 'T':
                rd_name = mylist.pop(-1)
                rdnamedb[rd_name] += rd_name + ";"
    ...
    with open(bkptsum_r) as fh:
        ...
        for line in fh:
            ...
            if with_read_name == 'T':
                rd_name = mylist.pop(-1)
                rdnamedb_right[rd_name] += rd_name + ";"
            try:
                (chrid, loci, croptype, coverage) = mylist
            except ValueError:
                logging.debug(line)
            ...
            for right in (lpe.right.start, lpe.right.end):
                ...
                for i in range(right - breakpoint_flanking, right + breakpoint_flanking):
                    try:
                        ...
                        if with_read_name == 'T':
                            rd += rdnamedb_right["_".join([rchr, str(i)])]
                    except KeyError:
                        pass
                etc += str(sum) + "\t"
                if with_read_name == 'T':

...
ParaAT.pl -h $ORTHO -n $CDS -a $PEP -p proc -o $ORTHO.ParaAT.out -f axt -k -M {4} -m mafft
...
selectItem.pl 1 0,1 ../{0}       ../{2} > {0}.right.ortho
...
selectItem.pl -h    {0}.cross ../{0}.kaks >{0}.cross.kaks
    ...
    with open(filter_ks_file) as fh:
        for line in fh:
            if gene in line:
                ...
                result.append("{}\t{}\t{}".format(res_list[0], group, res_list[1]))

...
def bsub(cmd, queue='Q104C512G_X4', direct_submit='F', cpus=1, name='', submit='T', options='', node=''):
    """
...
def is_job_finished(joblist=None):
    ...
    for j in joblist:
        ...
        if re.search(r'{}.*\sDONE'.format(j), status) or \
                re.search(r'{}.*\sEXIT'.format(j), status) or \

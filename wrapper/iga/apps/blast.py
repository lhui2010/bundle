...
blastp  -num_threads {4} -db {0} -query {1}  -evalue {3} -outfmt 6 -out {5} {6}
"""


def blastp(ref=None, qry=None, threads=5, eval=1e-5, use_grid='T', output='-', other_param=''):
    r"""
    ...
    :return:
    """
    cmd = blastp_sh.format(ref, qry, qry, eval, threads, output, other_param)
    if use_grid == 'T':
...
""".format(bln, top_num, eval)
    ...
    firstn(sorted_qry, field=1, num_extract=top_num, print_to_screen='F', output=sorted_qry + '.top')
    firstn(sorted_ref, field=2, num_extract=top_num, print_to_screen='F', output=sorted_ref + '.top')
    sh("""cut -f1,2 {0}.sorted.qry.top {0}.sorted.ref.top | \
    sed "s/\t/--++--/" |sort |uniq -c | grep "2 " |awk '{{print $2}}' |sed "s/--++--/\t/" > {2}""".format(
        bln, top_num, output))

...
mkdir -p $DIR/train_snap2
cd $DIR/train_snap2
ln -s ../genome.all.merge.gff genome.all.gff
...
mv ${HMMDIR}/${MYSPECIES}_v${VERSION}.hmm ${HMMDIR}/${MYSPECIES}_v${VERSION}.hmm.bak

cp snap_v${VERSION}.hmm ${HMMDIR}/${MYSPECIES}_v${VERSION}.hmm

echo "Train SNAP completed succefully:"
echo "$DIR/train_snap2/snap_v${VERSION}.hmm"

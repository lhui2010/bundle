...
EST_GFF=$ROOT/step2_annotation/round1/extract_gff/est2genome_total.gff
PEP_GFF=$ROOT/step2_annotation/round1/extract_gff/protein2genome_total.gff
...
for i in `cat fa_list`
    do
       ...
       # bsub -J mk$i -o maker.$i.log -e maker.$i.err "maker -R $OPT maker_bopts.ctl maker_exe.ctl >>"
        bsub -q Q104C512G_X4 -J R2mk$i -o output.$i -e error.$i " export AUGUSTUS_CONFIG_PATH=/tmp/lh_config && maker $OPT maker_bopts.ctl maker_exe.ctl >> maker.$i.log 2>>maker.$i.err"
        sleep 1s
    done

...
cat input/peps/* >input/peps.fa
cat input/transcripts/* >input/transcripts.fa
...
split_fastav3.pl ref.fa  100
#perl $DIR/split_fasta_each.pl ref.fa  100
rm ref.fa
mv ref.fa._ fasta_partition

ls fasta_partition/*.fa |sed 's/.fa//g; s/fasta_partition\///g' >fa_list
...
est=${DIR}/input/transcripts.fa #set of ESTs or assembled mRNA-seq in fasta format
protein=${DIR}/input/peps.fa  #protein sequence file in fasta format (i.e. from mutiple organisms)
...
est_gff=${DIR}/input/lncRNA_total.gff #aligned ESTs or mRNA-seq from an external GFF3 file
...
rmlib=${DIR}/step1_repeatmask/ref-families.fa #provide an organism specific repeat library in fasta format for RepeatMasker
...
#3.01.02
        bsub -q Q104C512G_X4 -J mk$i -o maker.$i.log -e maker.$i.err " export AUGUSTUS_CONFIG_PATH=/tmp/lh_config && /ds3200_1/users_root/yitingshuang/lh/bin/maker/bin/maker  $OPT maker_bopts.ctl maker_exe.ctl"
        sleep 2s

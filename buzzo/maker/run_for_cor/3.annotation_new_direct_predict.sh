...
EST_GFF=$ROOT/input/falcon_est.gff
PEP_GFF=$ROOT/input/falcon_peps.gff
RM_GFF=${ROOT}/step1_repeatmask/Full_mask/full_mask.complex.reformat.gff3

##generating CTL files
mkdir -p $ROOT/step2_annotation
...
for i in `cat fa_list`
    do
       ...
       # bsub -J mk$i -o maker.$i.log -e maker.$i.err "maker -R $OPT maker_bopts.ctl maker_exe.ctl >>"
        bsub -q Q104C512G_X4 -J R1mk$i -o output.$i -e error.$i "export AUGUSTUS_CONFIG_PATH=/tmp/lh_config && maker $OPT maker_bopts.ctl maker_exe.ctl >> maker.$i.log 2>>maker.$i.err"
        sleep 1s
    done

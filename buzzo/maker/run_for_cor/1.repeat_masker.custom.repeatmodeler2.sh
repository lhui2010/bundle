...
export PATH=~/lh/anaconda3/envs/repeatmodeler/bin/:$PATH
export PERL5LIB=""
bsub -q Q104C512G_X4  -o output.%J -e error.%J "BuildDatabase -name ref -engine ncbi ref.fa &&RepeatModeler -LTRStruct -engine ncbi -pa $THREADS -database ref  && RepeatMasker -lib ref-families.fa ref.fa -pa $THREADS -dir custom_lib.out" |awk '{print $2}' | sed 's/>//;s/<//' >>job.id

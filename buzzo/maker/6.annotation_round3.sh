...
for i in `cat fa_list`
    do
       ...
       # bsub -J mk$i -o maker.$i.log -e maker.$i.err "maker -R $OPT maker_bopts.ctl maker_exe.ctl >>"
        bsub -q Q104C512G_X4 -J R3mk$i -o output.$i -e error.$i " export AUGUSTUS_CONFIG_PATH=/tmp/lh_config && maker $OPT maker_bopts.ctl maker_exe.ctl >> maker.$i.log 2>>maker.$i.err"
        sleep 10s

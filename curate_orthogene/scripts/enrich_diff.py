...
#TODO
#parser = argparse.ArgumentParser(description='Perform GNU diff -y on specific column(default 4, name field of bed) of two files')
#parser.add_argument('QRY', type=str, nargs = 1,
#                    help='query file')
#parser.add_argument('REF', type=str, nargs = 1,
#                    help='reference file')
#parser.add_argument('COL', type=str, nargs = 1, default = 4,
#                    help='the column to compare the two files')
#parser.add_argument('--version', action='version', version='%(prog)s 0.1')
#args = parser.parse_args()

#zero based column number
column = sys.argv[3]
oscmd = 'for i in ' + sys.argv[1] + ' ' + sys.argv[2] + '; do awk \'{print $' + column + '}\' ${i} | sed \'s/.*#//\' > ${i}.id; done'
os.system(oscmd)
...
linesA = []
linesB = []
#Number of fields in A and B
sizeA = 6
sizeB = 6
syntag = ''
with open(sys.argv[1]) as fh:
    linesA = fh.readlines() 
sizeA = len(linesA[-3].split("\t"))
#linesA = ["\t".join(e.split()[7:13]) for e in linesA]

with open(sys.argv[2]) as fh:
    linesB = fh.readlines() 
#syntag = linesB[-1].split()[6]
#This variable used to represent which gene belong to which syntenic region
sizeB = len(linesB[0].split("\t"))
syntag = linesB[0].split()[0]
...
#Whether to move to next iterator of linesA or liensB
        ...
        countB = 1
#The 
        enriched_A = '\t'.join([''] * sizeA)
        diff_status = '='
        enriched_B  = '\t'.join([''] * sizeB)

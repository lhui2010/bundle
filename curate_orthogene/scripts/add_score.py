...
class teBedIO(BedIO):
    def __init__(self, filebed):
        ...
        with open(filebed) as fh:
            for line in fh:
                mylist = line.rstrip('\n').split('\t')
                (self.chr, start, end, gene_name, self.this_score, self.this_strand) = mylist[0:6]
                """Ignore lines with empty name"""
                if gene_name == "":
                    continue
                bed_key = self.chr + start + end + gene_name + self.this_strand
                ...
                self.start[bed_key] = start
                self.end[bed_key] = end

    def filter_bed(self, len_cutoff):
        for k in self.name:
            name = self.name[k]
            if(int(self.end[k]) - int(self.start[k]) < len_cutoff):
                del(self.bed_line[k])
...
#Gene's score
            ...
            if('#' in name):
                ...
                for fam in score_dict:
#                fam = name[0:3]
#                if fam in score_dict:
                    if fam in name:
                        this_score = score_dict[fam]

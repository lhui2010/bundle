...
############ Do what you want to do #####################
		...
		{
			if(!exists $promotor_gene_info{$i})
			{ $promotor_gene_info{$i} = $info; }
		}
	...
	while(<INFile2>)
	{
		chomp;
		my $temp = $_;
		if(exists $promotor_gene_info{$temp})
		{ print OutFile "$temp\t$promotor_gene_info{$temp}\n";}

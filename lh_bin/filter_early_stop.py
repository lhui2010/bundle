...
#rename genome_dict
    for old_key in raw_dict.keys():
        this_seq = re.sub(r'\*$', '',raw_dict[old_key].seq.__str__()) 
        if(re.search(r'\*|U', this_seq)):
            stop_dict[old_key] = raw_dict[old_key]
        else:
    ...
    with open(ffasta + ".contains_Stop.fa", "w") as output_handle:
        for k in stop_dict.keys():
            SeqIO.write(stop_dict[k], output_handle, "fasta")
